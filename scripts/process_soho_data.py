...
def read_and_process(file_path: str) -> pd.DataFrame:
    ...
    sub_df = df[columns_to_keep].copy()
    return sub_df
...
def main():
    ...
    input_dir = "/shared/raw/soho"
    output_dir = "/shared/processed/soho"

    all_file_list = glob.glob(f"{input_dir}/**/*.00", recursive=True)
    all_file_list.sort()
    print(f"Found {len(all_file_list)} files")


    soho_euv_data_grid_df = process_soho_data(all_file_list)

    #if output_dir does not exist, create it
    os.makedirs(output_dir, exist_ok=True)
    output_filename = os.path.join(output_dir,'soho_data.csv')
    soho_euv_data_grid_df.to_csv(output_filename, index=False)

def process_soho_data(all_file_list: list[str]):

    input_dataframes = map(read_and_process, all_file_list)
    soho_euv_data_df = pd.concat(input_dataframes, ignore_index=True)
