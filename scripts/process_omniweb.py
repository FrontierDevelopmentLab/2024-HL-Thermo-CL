...
def define_dic_invalid_and_names():
...
def process_one_omniweb_file(file_path):
    ...
    return data_omni_magnetic_field, data_omni_solar_wind_velocity, data_omni_indices

def main():

    input_dir='/shared/raw/omniweb'
    output_dir='/shared/processed/omniweb'

    files = os.listdir(input_dir)
    files = sorted([os.path.join(input_dir,f) for f in files if f.endswith('.txt') and f.startswith('data_')])
    ...
    for file_path in files:
        ...
        file_date = file_path.split('/')[-1].split('_')[-1].split(".")[0]
        print(f'saving to file, with file date: {file_date}')

        magnetic_field_output_file_name = os.path.join(output_dir,f'magnetic_field_omni_{file_date}.csv')
        data_omni_magnetic_field.to_csv(magnetic_field_output_file_name, index=False)

        solar_wind_output_file_name = os.path.join(output_dir,f'solar_wind_velocity_omni_{file_date}.csv')
        data_omni_solar_wind_velocity.to_csv(solar_wind_output_file_name, index=False)
        
        indices_output_file_name = os.path.join(output_dir,f'indices_omni_{file_date}.csv')
        data_omni_indices.to_csv(indices_output_file_name, index=False)
        print('done')
